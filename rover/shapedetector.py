...
class ShapeDetector:
	...
	def get_shapes_coordinates(self, image):
		...
		if max(image.shape) > self.IMAGE_MAX_WH_VIRTUAL_SCENARIO:
			resize_ratiox = float(self.IMAGE_MAX_WH_VIRTUAL_SCENARIO) / image.shape[0]
			resize_ratioy = float(self.IMAGE_MAX_WH_VIRTUAL_SCENARIO) / image.shape[1]
			image = cv2.resize(image, (0, 0),fx=resize_ratiox,fy=resize_ratioy,interpolation=cv2.INTER_AREA)
		...
		#sd = ShapeDetector()
		#only for obstacles, we want to know if it's horizontal or not
		orentation_obs = []
		...
		for c in cnts:
			...
			peri = cv2.arcLength(c, True)
			approx = cv2.approxPolyDP(c, 0.04 * peri, True)
			shape = self.detect_s(approx)
			
			if(len(approx) == 4):
				(x, y, w, h) = cv2.boundingRect(approx)
				if w>h:
					orentation_obs.append(1) # horizontal
				else:
					orentation_obs.append(0) # vertical
				#print((x*ratio, y*ratio, w*ratio, h*ratio))
				#ar = w / float(h)
		...
		return orentation_obs,image, obstacles_scenario, targets_scenario, starting_point
